...
def display():
    global x_pos,y_pos,z_pos, angle, isOrtho, tire_angle
    ...
    # Car
    glPushMatrix()
    glTranslate(-30 + car_offset, 0, 15) 
    drawCar()

    # Draw Tires
    glPushMatrix() ### Front right tire
    glTranslate(2, 0, 2)
    glPushMatrix() ### Rotate
    glRotated(-tire_angle, 0, 0, 1)
    drawTire()
    glPopMatrix()
    glPushMatrix() ### Front left tire
    glTranslate(0, 0, -4)
    glPushMatrix() ### Rotate
    glRotated(-tire_angle, 0, 0, 1)
    drawTire()
    glPopMatrix()
    glPopMatrix ### Back right tire
    glTranslate(-4, 0, 0)
    glPushMatrix()
    glRotated(-tire_angle, 0, 0, 1)
    drawTire()
    ...
    drawTire()
    glPopMatrix()
    glPopMatrix() ### MATRIX on stack is 
...
def keyboard(key, x, y):
    global x_pos,y_pos,z_pos, angle, isOrtho, car_offset
    if key == chr(27):
        import sys
        sys.exit(0)
